...
// Test that the proper registrations are generated for a NodePort type.
func TestServiceResource_nodePort_nodeAnnotationSync(t *testing.T) {
	t.Parallel()
	require := require.New(t)
	client := fake.NewSimpleClientset()
	syncer := &TestSyncer{}
	...
	closer := controller.TestControllerRun(&ServiceResource{
		...
		Client:       client,
		Syncer:       syncer,
		NodePortSync: NodeFQDNAnnotation,
	})
	defer closer()
	...
	_, err := client.CoreV1().Nodes().Create(&apiv1.Node{
		ObjectMeta: metav1.ObjectMeta{
			Name:        node1,
			Annotations: map[string]string{"consul.hashicorp.com/node-fqdn": "node1.hashicorp.com"},
		},
	...
	_, err = client.CoreV1().Nodes().Create(&apiv1.Node{
		ObjectMeta: metav1.ObjectMeta{
			Name:        node2,
			Annotations: map[string]string{"consul.hashicorp.com/node-fqdn": "node2.hashicorp.com"},
		},
	...
	require.Len(actual, 2)
	require.Equal("foo", actual[0].Service.Service)
	require.Equal("node1.hashicorp.com", actual[0].Service.Address)
	require.Equal(30000, actual[0].Service.Port)
	require.Equal("k8s-sync", actual[0].Node)
	require.Equal("foo", actual[1].Service.Service)
	require.Equal("node2.hashicorp.com", actual[1].Service.Address)
	require.Equal(30000, actual[1].Service.Port)
	require.Equal("k8s-sync", actual[1].Node)
	require.NotEqual(actual[0].Service.ID, actual[1].Service.ID)
}
